...
// ── Sidebar ────────────────────────────────────────────────────────────────────
function Sidebar({ newOrdersCount, userEmail }: { newOrdersCount: number; userEmail?: string }) {
  const NAV = [
  ...
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: '10px 8px' }}>
        ...
        <NavSection label="CANAUX DE VENTE">
      ...
      <div style={{ padding: '12px 14px', borderTop: '1px solid #F1F1F1', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: '#8A8A8A' }}>Administrateur</div>
        </div>
        {/* Déconnexion via form POST — fonctionne sans JS */}
        <form action="/api/users/logout" method="POST" style={{ flexShrink: 0 }}>
          <button type="submit" title="Déconnexion" style={{
...
function NavSection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ fontSize: 10, fontWeight: 600, color: '#B0B0B0', letterSpacing: '0.08em', padding: '8px 8px 4px', textTransform: 'uppercase' }}>{label}</div>
      {children}
    </div>
  )
}
...
function NavItem({ href, icon, label, badge, external }: { href: string; icon: React.ReactNode; label: string; badge?: number; external?: boolean }) {
  return (
    <Link href={href} target={external ? '_blank' : undefined} className="admin-nav-item" style={{
      display: 'flex', alignItems: 'center', gap: 10,
      padding: '8px 10px', borderRadius: 8,
      textDecoration: 'none', color: '#3D3D3D',
...
export default async function BoutiqueAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // AUTH TEMPORAIREMENT DÉSACTIVÉE — à remettre avant la mise en production finale
  // if (!session?.user) {
  //   return (
